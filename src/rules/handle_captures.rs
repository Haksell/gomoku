...
use crate::{constants::DIRECTIONS8, player::Player, Model};
    ...
    let total_captures = DIRECTIONS8
        ...
        .filter(|&&(dx, dy)| {
            ...
            if is_capture {
                ...
                model.board[(y + dy) as usize][(x + dx) as usize] = Player::None;
                model.board[(y + 2 * dy) as usize][(x + 2 * dx) as usize] = Player::None;
            }
            is_capture
        })
        .count();
